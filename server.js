...
app.post('/api/apps', async (req, res) => {
  ...
  const runResult = await runApp(appId, newApp);
  if (runResult.ok) {
    newApp.running = true;
    newApp.port = runResult.port;
  }

  res.status(201).json({ ...newApp, running: runResult.ok });
});
...
const COMMON_ENV_KEYS = ['TELEGRAM_BOT_TOKEN', 'BOT_TOKEN', 'API_KEY', 'API_SECRET', 'DATABASE_URL', 'PORT', 'NODE_ENV', 'SECRET_KEY', 'OPENAI_API_KEY', 'DISCORD_TOKEN', 'CHECK_INTERVAL'];
...
function suggestEnvKeys(repoPath) {
  ...
  COMMON_ENV_KEYS.forEach((k) => keys.add(k));
  try {
...
app.get('/api/apps/:id/health', (req, res) => {
  ...
  const opts = { hostname: '127.0.0.1', port, path: '/', method: 'GET', timeout: 3000 };
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (msg) => {
    try {
      ...
      const data = raw.startsWith('{') ? JSON.parse(raw) : { type: 'input', data: raw };
      if (data.type === 'init' && data.appId) {
        const repoPath = path.join(REPOS_DIR, data.appId);
        if (!fs.existsSync(repoPath) || !path.resolve(repoPath).startsWith(path.resolve(REPOS_DIR))) {
          ws.send(JSON.stringify({ type: 'error', data: 'Geçersiz uygulama' }));
          return;
        }
        if (!pty) {
          ws.send(JSON.stringify({ type: 'error', data: 'Terminal desteklenmiyor (node-pty yüklü değil)' }));
          return;
        }
        const shell = process.platform === 'win32' ? 'powershell.exe' : process.env.SHELL || 'bash';
        term = pty.spawn(shell, [], { cwd: repoPath, cols: 80, rows: 24, name: 'xterm-256color' });
        term.onData((d) => ws.send(JSON.stringify({ type: 'output', data: d })));
        term.onExit(() => { term = null; ws.send(JSON.stringify({ type: 'exit' })); });
        ws.send(JSON.stringify({ type: 'ready' }));
      } else if (data.type === 'input' && term) {
        term.write(data.data || '');
      } else if (data.type === 'resize' && term && typeof data.cols === 'number' && typeof data.rows === 'number') {
